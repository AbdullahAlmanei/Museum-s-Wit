...
export class ContainerFrame extends BaseScriptComponent {
  ...
  onAwake() {
    ...
    this.closeButton.onTrigger.add(() => {
      ...
      // --- find the child scene-object called “aicontroller” ---
      const childCount = this.parent.getChildrenCount();
      let  aiObj       = null;
      for (let i = 0; i < childCount; i++) {
          const c = this.parent.getChild(i);
          if (c.name === "aicontroller") {      // case-sensitive
              aiObj = c;
              break;
          }
      }
      if (!aiObj) {
          print("Could not find aicontroller under parent");
          return;
      }
  
      // --- grab its ScriptComponent and call stopChat() ---
      const aiScript = aiObj.getComponent("Component.ScriptComponent");
      if (aiScript && aiScript.api && aiScript.api.stopChat) {
          aiScript.api.stopChat();
      } else {
          print("aicontroller script is missing or stopChat not exported");
      }
  });

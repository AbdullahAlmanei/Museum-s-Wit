...
//@input SceneObject buttonMesh

//@input Asset.Material idleMaterial   {"label":"Idle / default"}
//@input Asset.Material loadingMaterial {"label":"Loading / highlight"}
...
//@input Component.ScriptComponent dialogueCtrl   // drag DialoguePanel here
...
async function processTexture() {
  ...
  const meshVisual = script.buttonSingleDetection.getComponent("Component.MeshVisual");
  if (!meshVisual) {
  ...
  {meshVisual.addMaterial(script.loadingMaterial);}
  ...
  const url = 'https://meet-gopher-moving.ngrok-free.app/test'

  print("sending request")
  const secondTexture = getTextureToProcess();

    // Create a request with the image data included
    const request = new Request(url, {
        method: 'POST',
        body: JSON.stringify({
            prompt: "what's in the scene. ONE SENTENCE!",
            image: encodedImage  // Send the full base64 image data
        }),
        headers: {
            'Content-Type': 'application/json'
        }

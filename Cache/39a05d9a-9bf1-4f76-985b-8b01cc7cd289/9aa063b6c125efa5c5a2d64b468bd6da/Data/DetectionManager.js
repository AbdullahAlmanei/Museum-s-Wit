...
async function handleTriggerEndSingleDetection(event) {
  ...
  try {
    ...
    script.dialogueCtrl.api.open("Hey i am saying stuff lollloll hehe, haga? lol what");
  ...
  const secondTexture = getTextureToProcess();
    ...
    if (response.status === 200) {
      ...
      if (responseJson && responseJson.choices && 
          ...
          script.diagText.text = responseJson.choices[0].message.content;
      } else {
          // Otherwise just show a success message
          script.diagText.text = "Pondering on art...";
      }
    }
